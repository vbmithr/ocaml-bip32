...
module Make (Crypto : CRYPTO) = struct
  ...
  let pp_k ctx ppf k =
    Hex.pp ppf (Key.to_bytes ctx k |> Hex.of_bigstring)
  ...
  let pp : type a. Context.t -> Format.formatter -> a t -> unit
    ...
    Format.fprintf ppf "@[<hov 0>key %a@ chaincode %a@ path %a@ parent %a@]"
      (pp_k ctx) k
      Hex.pp (Hex.of_bigstring c)
      (Format.pp_print_list
         ~pp_sep:(fun ppf () -> Format.pp_print_char ppf '/')
         pp_print_path) (List.rev path)
      Hex.pp (Hex.of_bigstring parent)
  ...
  let neuterize : type a. Context.t -> a t -> Key.public t = fun ctx k ->
    { k with k = Key.neuterize_exn ctx k.k }

  let derive : type a. Context.t -> a t -> int32 -> a t = fun ctx { k ; c = key ; path ; _ } i ->
  ...
  let derive_path ctx k path =
    ListLabels.fold_left path ~init:k ~f:(derive ctx)
  ...
  let secret_of_bytes ctx cs =
    try Some (secret_of_bytes_exn ctx cs) with _ -> None
  let public_of_bytes ctx cs =
    try Some (public_of_bytes_exn ctx cs) with _ -> None
  ...
  let to_bigstring : type a. Context.t -> a t -> Bigstringaf.t = fun ctx { k ; c ; path ; parent } ->
    let buf = Bigstringaf.create 74 in (* not initialized to zero! *)
    Bigstringaf.set buf 0 (List.length path |> Char.chr) ;
    Bigstringaf.blit parent ~src_off:0 buf ~dst_off:1 ~len:4 ;
    Bigstringaf.set_int32_be buf 5 (match path with [] -> 0l  | i :: _ -> i) ;
