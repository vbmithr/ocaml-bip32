...
module Crypto = struct
  ...
  let hmac_sha512 ~key s =
    ...
    SHA512.hmac_bigstring ~key s
    |> SHA512.to_raw_string
    |> fun s -> Bigstringaf.of_string s ~off:0 ~len:(String.length s)
  ;;

  let sha256 s =
    SHA256.digest_bigstring s
    |> SHA256.to_raw_string
    |> fun s -> Bigstringaf.of_string s ~off:0 ~len:(String.length s)
  ;;

  let ripemd160 s =
    RMD160.digest_bigstring s
    |> RMD160.to_raw_string
    |> fun s -> Bigstringaf.of_string s ~off:0 ~len:(String.length s)
  ;;
end
...
let process ctx seed paths expected =
  ...
  ListLabels.fold_left2 ~init:m paths (List.tl expected) ~f:(fun a p e ->
...
let harden i = Int32.logor 0x8000_0000l i
...
let vectors =
  [ test_case "v1" `Quick v1; test_case "v2" `Quick v2; test_case "v3" `Quick v3 ]
;;

let () =
  let ctx = Secp256k1.Context.(create []) in
  let ctx = Cmdliner.Term.const ctx in
  run_with_args "Bip32" ctx [ "vectors", vectors ]
;;
